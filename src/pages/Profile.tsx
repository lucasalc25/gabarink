        ...
        <div className="relative flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex flex-wrap gap-3 mt-4 justify-center md:justify-start">
              {(Object.keys(user.stats) as AreaENEM[]).map((area) => (
                <div key={area} className="flex items-center gap-2 bg-white/5 pr-3 rounded-full border border-white/5">
                  <AreaBadge area={area} score={user.stats[area].score} size={28} />
                  <div className="text-left">
                    <p className="text-[9px] font-black opacity-50 uppercase leading-none">{area}</p>
                    <p className="text-[10px] font-bold text-primary-light">
                      {LEAGUES_CONFIG[area][Math.floor(user.stats[area].score / 100) * 100]}
                    </p>
                  </div>
                </div>
              ))}
            </div>
            <div className="h-3 rounded-full bg-secondary overflow-hidden max-w-md mt-4">
              <div className="h-full liquid-bar" style={{ width: `${xpPct}%` }} />
            </div>
            <p className="text-xs text-muted-foreground mt-1">{user.xp}/{user.xpToNextLevel} XP TOTAL</p>
          </div>
        </div>
