...
const QuizPlayer = () => {
  ...
  if (loading) return <AppLayout><div className="flex items-center justify-center min-h-[60vh] text-muted-foreground animate-pulse font-black uppercase tracking-widest">Carregando quiz...</div></AppLayout>;
  
  if (error || !quiz) {
    return (
      <AppLayout>
        <div className="max-w-xl mx-auto glass-card p-12 text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground">O conteúdo que você está tentando acessar não está disponível no momento.</p>
          </div>
          <Button onClick={() => nav("/home")} className="bg-primary rounded-xl px-8 font-bold">Voltar para Início</Button>
        </div>
      </AppLayout>
    );
  }
